 ...
 * <a href = "http://introcs.cs.princeton.edu/java/stdlib/StdDraw.java">StdDraw.
 ...
 * @author Kevin Wayne
 */
public final class StdDrawInput implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
	...
	private static StdDrawInput std = new StdDrawInput();
	...
	/**
	 * Draws a polygon with the vertices (<em>x</em><sub>0</sub>, <em>y</em>
	 * <sub>0</sub>), (<em>x</em><sub>1</sub>, <em>y</em><sub>1</sub>), ..., (
	 * <em>x</em><sub><em>n</em>&minus;1</sub>, <em>y</em><sub><em>n</em>
	 * &minus;1</sub>).
	 *
	 * @param x
	...
	private static BufferedImage getImage(String filename) {
		...
		try {
			URL url = StdDrawInput.class.getResource(filename);
			BufferedImage image = ImageIO.read(url);
			return image;
		} catch (IOException e) {
			// ignore
	...
	public void actionPerformed(ActionEvent e) {
		FileDialog chooser = new FileDialog(StdDrawInput.frame, "Use a .png or .jpg extension", FileDialog.SAVE);
		chooser.setVisible(true);
		String filename = chooser.getFile();
		if (filename != null) {
			StdDrawInput.save(chooser.getDirectory() + File.separator + chooser.getFile());
		}
	}
	...
	public void mousePressed(MouseEvent e) {
		synchronized (mouseLock) {
			mouseX = StdDrawInput.userX(e.getX());
			mouseY = StdDrawInput.userY(e.getY());
			mousePressed = true;
		}
	}
	 ...
	 */
	public static void main(String[] args) {
		StdDrawInput.square(.2, .8, .1);
		StdDrawInput.filledSquare(.8, .8, .2);
		StdDrawInput.circle(.8, .2, .2);

		StdDrawInput.setPenColor(StdDrawInput.BOOK_RED);
		StdDrawInput.setPenRadius(.02);
		StdDrawInput.arc(.8, .2, .1, 200, 45);

		// draw a blue diamond
		StdDrawInput.setPenRadius();
		StdDrawInput.setPenColor(StdDrawInput.BOOK_BLUE);
		double[] x = { .1, .2, .3, .2 };
		double[] y = { .2, .3, .2, .1 };
		StdDrawInput.filledPolygon(x, y);

		// text
		StdDrawInput.setPenColor(StdDrawInput.BLACK);
		StdDrawInput.text(0.2, 0.5, "black text");
		StdDrawInput.setPenColor(StdDrawInput.WHITE);
		StdDrawInput.text(0.8, 0.8, "white text");
	}
